...
from codigouni import (
    CANParser,
    DataParser,
    DataRecorder,
    InMemoryRecorder,
    JSONLoggingRecorderWrapper,
    JSONUARTParser,
    MockUARTDevice,
    SensorReading,
    ThreadSafeCircularRecorder,
    UARTDevice,
)
...
class DriverModernizadoUART:
    def __init__(
        self, device: UARTDevice, parser: DataParser, recorder: DataRecorder
    ) -> None:
        self._device = device
        self._parser = parser
...
def test_driver_modernizado_uart() -> None:
    ...
    driver = DriverModernizadoUART(
        device=device_simulado, parser=parser_json, recorder=recorder_memoria
    )
...
def test_driver_extension_alto_potencial() -> None:
    ...
    driver = DriverModernizadoUART(
        device=device_can, parser=parser_can, recorder=recorder_con_logging
    )
