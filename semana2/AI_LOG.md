...
Esta bitácora documenta cómo se utilizó el asistente de IA  durante el Sprint 1 para traducir requerimientos de negocio a historias de usuario, configurar el entorno de calidad y debugear herramientas de testing.

##  Entrada 1: Traducción de Requerimientos a User Stories y Gherkin
 Se necesitaba desglosar el sistema de monitoreo IoT (bodega industrial) en al menos 10 Historias de Usuario estructuradas y priorizadas.
 **Prompt utilizado:** *"¿Cómo redacto el Product Backlog para un sistema IoT usando priorización MoSCoW y criterios de aceptación Gherkin?"*

**Respuesta de la IA:** La IA me guio para utilizar el formato estándar "Como [rol], quiero [X] para [beneficio]" y a escribir los criterios en formato `Dado / Cuando / Entonces` (Given/When/Then). Utilice ejemplos que le pedi para apoyarme y realizar la actividad 

##  Entrada 2: Configuración del Entorno de Calidad (pyproject.toml y Coverage)
Para cumplir con la "Definition of Done" del Sprint, el código debía tener reglas estrictas de Ruff, Mypy y fallar si la cobertura de pruebas era menor al 80%.
 **Prompt utilizado:** *"Me marca este error al correr pytest: unrecognized arguments: --cov=. --cov-fail-under=80 y Configura pyproject.toml: reglas de ruff (E, F, I, UP, B), pytest con --cov-fail-under=80, mypy con disallow_untyped_defs. "* 

 **Respuesta de la IA :** La IA me ayudó a entender que el archivo `pyproject.toml` orquesta las reglas, pero yo no tenía instalado el motor que hace el cálculo. Me indicó instalar el plugin `pytest-cov`y con respecto al segundo prompt me otorgo un codigo que utilice en su totalidad ya que cumplia con los requerimientos de la sesion.

##  Entrada 3: Depuración de Errores Silenciosos en TDD
 Durante el desarrollo de la historia US-01 (SensorReading) usando TDD, el reporte de Pytest mostraba un extraño 0% de cobertura en mis archivos de prueba, a pesar de que el código lógico era correcto.
 **Prompt utilizado:** *"En test_US_01.py me sigue marcando 0 de coverage. Este es el código: import pytest..."*
 
 **Respuesta de la IA:** La IA detectó dos problemas que un humano difícilmente vería a simple vista: el uso de guiones medios en módulos Python (`US-01.py`) y la presencia de caracteres invisibles inválidos (espacios duros `U+00A0`) insertados al copiar código.
