...
class AnomalyDetector:
    def __init__(self, alert_strategy: AlertStrategy, temp_threshold: float, hum_threshold: float):
    ...
    def check(self, reading: SensorReading) -> None:
        if reading.reading_type == "temperature" and reading.value > self._temp_threshold:
            self._alert.send_alert(f"CRITICAL TEMP: {reading.value}°C from {reading.sensor_id}")
            
        elif reading.reading_type == "humidity" and reading.value > self._hum_threshold:
            self._alert.send_alert(f"CRITICAL HUM: {reading.value}% from {reading.sensor_id}")