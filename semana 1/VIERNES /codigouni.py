    ...
    def parse(self, raw_data: str) -> SensorReading:
        try:
            ...
            return SensorReading(
                sensor_id=data.get("id", "UNKNOWN"), value=float(data.get("val", 0.0))
            )
        except json.JSONDecodeError:
            # Manejo básico si el cable metió ruido y el JSON llega roto
          
            raise ValueError("Trama UART corrupta o formato inválido") from None
...
class CANParser:
    ...
    def parse(self, raw_data: str) -> SensorReading:
        try:
            ...
            raise ValueError("Trama CAN corrupta o formato inválido") from e
