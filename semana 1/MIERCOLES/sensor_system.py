...
class DataLogger:
    ...
    def save(self, reading: SensorReading) -> None:
        ...
        print(
            f"Guardando lectura de {reading.sensor_id}: {reading.value} "
        )
...
#  Agregamos EmailAlert SIN modificar las clases de arriba ni la clase que las use.
...
class AnomalyDetector:
    ...
    def check(self, reading: SensorReading) -> None:
        if reading.value > self._threshold:
            self._alert.send(
                f"Anomalía detectada en {reading.sensor_id}. Valor: {reading.value}"
            )
